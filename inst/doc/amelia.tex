...
Ghana in a given year was somwhere around $3.5\%$, but we have some
...
\hline
46 &  \adjparbox{5in}{The arugment list you provided was invalid.  The
...
\label{sec:cmdline}
\subsection{\texttt{amelia}: Multiple Imputaton of Incomplete
  Multivariate Data}
...
\subsubsection*{Arguments}
    \begin{itemize}
      \item \texttt{data}:incomplete data, organized into either a
        data frame or a matrix.
  
       \item \texttt{m}: number of imputed datasets to create.
  
       \item \texttt{p2s} :an interger value taking either 0 for no
      ...
      \item \texttt{idvars}: a vector of column numbers or column
        names that indicates identification variables.
    
      \item \texttt{logs}:a vector of column numbers or column names
        that refer to variables that require log-linear
        transformation.
    
      \item \texttt{ts}:number or name indicating column of the time
        series variable.
  
      \item \texttt{cs}: number or name indicating column of the cross
        section variable.

  
      \item \texttt{casepri} :indicator matrix of size \emph{kxk}
        (where \emph{k} is the number of cases) for the degree of
        similarity between two cases.  For example, the [2,3] entry
        would indicate how similar cases 2 and 3 were.  The indicators
        can be 0, 1, 2, or 3.  Values should only appear in the upper
        triangle, as values in the lower triangle are ignored.
    
      \item \texttt{means}: a matrix with the same dimensions as the
        data where each cell contains a prior belief about the mean
        value of that cell in the data.  For example, the values in
        \texttt{means[2,3]} would be your prior belief about the mean of
        data[2,3].  Cell should be \texttt{NA} if no prior exists. If
        \texttt{means} is set, \texttt{sds} must be set as well.

    
      \item \texttt{sds}: a matrix with the same dimensions as the
        data where each cell contains a prior belief about the
        stardard deviation around the prior mean of that cell in the
        data.  For example, the values in \texttt{sds[2,3]} would be
        your prior belief about the \texttt{means[2,3]} of
        \texttt{data[2,3]}.  Cell should be \texttt{NA} if no prior
        exists. If \texttt{sds} is set, \texttt{means} must be set as
        well.
    
      \item \texttt{mins}: a matrix with the same dimensions as the
        data, used for range priors.  A cell in this matrix contains a
        prior belief about a minimum value for that cell in the data.
        For example, the values in \texttt{mins[2,3]} would be your
        prior belief about the minimum of \texttt{data[2,3]}.  If
        \texttt{mins} is set, \texttt{maxs} and \texttt{conf} must be set as
        well.
    
      \item \texttt{maxs}: a matrix with the same dimensions as the
        data, used for range priors.  A cell in this matrix contains a
        prior belief about a maximum value for that cell in the data.
        For example, the values in \texttt{maxs[2,3]} would be your
        prior belief about the maximum of \texttt{data[2,3]}.  If
        \texttt{maxs} is set, \texttt{mins} and \texttt{conf} must be set as
        well.
    
      \item \texttt{conf}: a matrix with the same dimensions as the
        data, used for range priors.  A cell in this matrix contains
        your prior confidence about the range prior for that cell in
        the data.  For example, the values in \texttt{maxs[2,3]} would
        be your prior belief about the maximum of \texttt{data[2,3]}.
        If \texttt{maxs} is set, \texttt{mins} and \texttt{conf} must be set
        as well.
    
      \item \texttt{empri}: number indicating level of the empirical
        (or ridge) prior.  This prior shinks the covariances of the
        data, but keeps the means and variances the same for problems
        of high missingness, small N's or large correlations among the
        variables.  Should be kept small; a reasonable upper bound is
        around 10\% of the rows of the data.
  
      \item \texttt{tolerance}: convergence requirement.
  
      \item \texttt{polytime}: integet between 0 and 3 indicating what
        power of polynomial should be included in the imputation model
        to account for the effects of time.  A setting of 0 would
        indicate no time effects, 1 would indicate linear time
      ...
      \item \texttt{sqrts}: a vector of numbers or names indicating
        columns in the data that should be transformed by a sqaure
      ...
      \item \texttt{ords}: a vector of numbers or names indicating
        columns in the data that are ordinal variables.
        
      \item \texttt{collect}: a logical value indicating whether or
        not garbage collection should occur during the imputations.
        Only set this to \texttt{TRUE} if you are experiencing memory
      ...
      \item \texttt{incheck}: a logical indicating whether or not the
        ...
        are extremely confident that your settings are non-problematic
        and you are trying to save computational time.
      ...
      \item \texttt{write.out}: a logical value indicating whether or
        ...
        comma-seperated value files.  If \texttt{TRUE}, Amelia will use
        the \texttt{outname} arugment as the file prefix.
        
      \item{arglist}: an output list from the \texttt{amelia} function
        or from a saved session from AmeliaView.  Values from this
        list take precendent over any individually set arugments.  See
...
Multiple imputation is a method for analyzing incomplete
  ...
  either data frame or matrix form and return \texttt{m} imputed datatsets
  ...
  interact, you can use the \texttt{casepri} setting.  If you have a prior
  ...
  with a 0 in the [50,7] location and a seperate \texttt{sds} matrix with
