...
%While the underlying mechanics of ${\mathfrak Amelia}$ consist of only
%a few functions, there are a number of ways to access these functions
%to produce imputations.  At its most basic, ${\mathfrak Amelia}$ is a
%library for the R statistical software package that makes the
%${\mathfrak Amelia}$ routine available in the R environment.  In
%addition, a Graphical User Interface (GUI) can access the same routine
%without the need for direct user coding.  In Windows, ${\mathfrak
%  Amelia}$ allows the user to bypass the use of R completely by
%detaching the GUI from R. In Linux, ${\mathfrak Amelia}$ can run from
%the command line or from the GUI, yet both require use of R.
%Currently, there we do not support a MacOS version of ${\mathfrak
%  Amelia}$, yet we hope to change this in the near future.  All
%versions of ${\mathfrak Amelia}$ require the R software, which is
%freely available at \url{http://www.r-project.org/}.  It compiles and
%runs on a wide variety of platforms including UNIX, Windows, and
%MacOS.  This manual will focus on the GUI version of ${\mathfrak
%  Amelia}$, although many of the ideas presented apply equally to the
%command-line version.  For help with the syntax of the command-line
%version, please refer to the help files included with the ${\mathfrak
%  Amelia}$ library.  These can be accessed at the R command prompt by
%typing
%  \begin{verbatim}
%  > help(``amelia'') .
%  \end{verbatim}
%In what follows we will describe the how a user would use the GUI
%version, but follow this with syntax to create the same procedure from
...
\begin{figure}[ht]
  \includegraphics[scale=.75]{case.png}
...
\label{sec:refobspri}
\begin{figure}[h]
  \includegraphics[scale=.75]{obspri.png}
