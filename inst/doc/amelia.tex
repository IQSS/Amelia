...
well, the imputation will down-weight the prior, and vice versa \citep{HonKin06}.  
...
For example, in the following example the first row constructs a prior mean of 500 with a standard deviation of 50 on the missing cell in row 42, column 3:
...
consistent across the entire matrix.  The last prior create in the above example specifies a row number of 0.  This indicates that the prior created should be applied to all missing values in this variable (here variable 2), unless another prior is specifically created for that observation (as in observation 46 in this example).
